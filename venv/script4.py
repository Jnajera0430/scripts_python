...
def config_var_global():
    ...
    print("Registrar coordenadas de la consola de transacciones: ")
    time.sleep(4)
    point_search_page = pyautogui.position()
...
def recostear_valor(num_Process:int):
...
try: 
    ...
    while True:       
        ...
        if start:
            ...
            primer_proceso = True
            acount = 2 
            click_change_page_search(point_search_page)
            search_valor_unitario()
            while proceso_abierto:
                if acount == 2:
                    click_change_page_console(point_console)
                    valor_final = int(input("Ingresa el precio unitario: "))
                    recostear_valor(acount) 
                    generar_valor()
                else:
                    recostear_valor(acount)
                    click_change_page_list(point_list_page)
